...
const numBalls        = document.querySelector('#ball-count'); // *ADDED* input for the number of balls
const rightProb       = document.querySelector('#prob-right'); // *ADDED* input for the rightward probability
...
function drawBoard() {
    ...
    async function dropBall() {
        ...
        // ADDED - Random Ball Color
        const darkening = Math.random();
        const saturation = 2*Math.random() - 1;
        let updated_color = chroma(BALL_COLOR).darken(darkening).saturate(saturation);
        circle.setAttribute('fill', updated_color);
        ...
        for(let i = 0; i < NUM_LEVELS-1; i++) {
            ...
            // Move the ball to the new location - REPLACED WITH ANIMATEX FUNCTION THAT INCLUDES EASE OUT

            // ADDED 
            await animateX(circle, x, y, DELAY_BETWEEN_PEGS / parseFloat(speedInput.value));

            const peg = pegs[row][col]; // The peg that the ball hit
            hitCounts[row][col]++; // Increment the hit count for this peg

            // ADDED - color scale for pegs
            let scale = chroma.scale(['#DDD', BALL_COLOR]);
            let fraction = hitCounts[row][col]/numBalls.value;
            peg.setAttribute('fill', scale(fraction));
        ...
        const newBarHeight = BAR_SCALE_FACTOR * finalColHitCount / numBalls.value; // The new height of the bar
        await endTime(circle, DELAY_WHEN_DROP / parseFloat(speedInput.value)); // ADDED - fading balls out
        await animatedRectHeight(actualBars[barIndex], newBarHeight, DELAY_WHEN_DROP / parseFloat(speedInput.value)); // Animate the change in height of the bar
    ...
    async function dropBalls() {
        ...
        /*ADDED*/
        numBalls.setAttribute('disabled', true);
        rightProb.setAttribute('disabled', true);
        ...
        // * ADDED *
        numBalls.removeAttribute('disabled');
        rightProb.removeAttribute('disabled');
    }
...
// ADDED
numBalls.addEventListener('input', redrawBoard);
rightProb.addEventListener('input', redrawBoard);
...
//ADDED - provided function that eases out animation
function easeOutQuad(t) {
     return t*(2-t); 
}   

//ADDED - provided function that eases in animation
function easeInQuad(t) { 
    return t*t; 
}

//ADDED - provided an animation to move circle downward and change opacity
async function endTime(circle, duration) {
    return new Promise((resolve, reject) => {
        ...
        function step() {
            ...
            const valueY = fromY + 20 * pos;
            circle.setAttribute('cy', valueY);
            circle.style.opacity = 1 - pct; 

            if (pct < 1) {
                requestAnimationFrame(step);
            } else {
                circle.setAttribute("cy", fromY + 20);
...
// ADDED - animating bar height
async function animatedRectHeight(rect, toHeight, duration) {
    return new Promise((resolve, reject) => {
        ...
        function step() {
            ...
            const newHeight = fromHeight + (toHeight - fromHeight) * easedProgress;
...
// ADDED - provided animation function on canvas
async function animateX(circle, toX, toY, duration) {
    return new Promise((resolve, reject) => {
      ...
      function step() {
        ...
        const valueX = fromX + (toX - fromX) * pos; 
        const valueY = fromY + (toY - fromY) * pos; 
